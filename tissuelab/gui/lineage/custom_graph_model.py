...
__revision__ = " $Id: custom_graph_model.py 3600 2012-05-27 20:14:46Z dbarbeau $ "
...
class Graph(Observed):
    ...
    def new_vertex(self, node=None, position=None):
        v = Vertex(node)
        self.add_vertex(v, node, position)
        return v

    def add_vertex(self, vertex, node=None, position=None):
        if vertex is None:
            return

        self.__vertices[self.__vidCounter] = vertex
        self.__node[self.__vidCounter] = node
