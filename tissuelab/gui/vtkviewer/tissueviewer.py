...
class TissueViewer(QtWidgets.QWidget):
    ...
    def _create_actions(self):
        self.action_auto_focus = QtWidgets.QAction(qicon("paraview/pqResetCamera32.png"), 'Auto focus', self)
        self.action_save_screenshot = QtWidgets.QAction(qicon("Crystal_Clear_app_camera.png"), 'Screenshot', self)
        ...
        for direction in list('xyz'):
            for sign, vtk_sign, symbol in (['positive', 'plus', '+'], ['negative', 'minus', '-']):
                ...
                action = QtWidgets.QAction(qicon(icon), 'Set view %s%s' % (symbol, direction.upper()), self)
        ...
        self.action_parallel_projection = QtWidgets.QAction(qicon("paraview/pqRotate32.png"), 'Parallel Projection', self)
    ...
    def toolbars(self):
        toolbar = QtWidgets.QToolBar("Tissue Viewer")
        toolbar.addActions([
